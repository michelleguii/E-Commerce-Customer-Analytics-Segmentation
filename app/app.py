...
with tab1:
  ...
  # Compact business-insight strip: adds hierarchy without changing the underlying analysis.
  top_market = (
  ...
  with col_left:
    with st.container(border=True):
      ...
      fig_monthly.update_layout(height=280, hovermode="x unified")
  ...
  with col_right:
    with st.container(border=True):
      ...
      fig_country.update_layout(showlegend=False, height=270, bargap=0.25)
  ...
  with col_left:
    with st.container(border=True):
      ...
      top_products["DisplayDescription"] = top_products["Description"].str.slice(0, 32).where(
          top_products["Description"].str.len() <= 32,
          top_products["Description"].str.slice(0, 29) + "..."
      )
      fig_products = px.bar(
          top_products,
          x="TotalAmount",
      ...
      fig_products.update_layout(height=285, margin=dict(l=8, r=8, t=24, b=8))
      apply_dark_chart_theme(fig_products)
      fig_products.update_yaxes(title_text="")
  ...
  with col_right:
    with st.container(border=True):
      ...
      fig_segment.update_layout(showlegend=False, height=240, bargap=0.25)
  ...
  with col_left:
    with st.container(border=True):
      ...
      fig_recency.update_layout(showlegend=False, height=255, bargap=0.25)
  ...
  with col_right:
    with st.container(border=True):
      ...
      fig_guest.update_layout(height=240, legend=dict(orientation="h", y=-0.06, x=0.5, xanchor="center"))
